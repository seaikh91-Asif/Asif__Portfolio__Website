...
const linkAction = () => {
	const navMenu = document.getElementById('nav-menu')
	// When we click on each nav__link, we remove the show-menu by clicking this link
	navMenu.classList.remove('show-menu')
}
...
let swiperProjects = new Swiper(".projects__container", {
    ...
    // Automatic slide change (Autoplay) add kora holo
    autoplay: {
        delay: 2000, 
...
const contactForm = document.getElementById('contact-form'),
      ...
      contactButton = contactForm.querySelector('button'); 
...
const sendEmail = (e) => {
    ...
    if (contactName.value === '' || contactEmail.value === '' || contactProject.value === '') {
        ...
        // 2. Button state change (Loading...)
        contactButton.disabled = true;
        contactButton.textContent = 'Sending...';

        emailjs.sendForm('service_99', 'template_95i87fp', '#contact-form', '2gH5hC4xO5ZlQxfiw')
        .then(() => {
            // 3. Success Response
        ...
        .finally(() => {
            // restore the button (error \ succesfull )
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    sr.reveal(`.home__data, .home__blob`, { delay: 500 })
    ...
    sr.reveal(`.projects__container, .testimonials__container, .footer__container`)
})
